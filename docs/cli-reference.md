...
```mermaid
flowchart TD
    Settings["MIDI 入力・新規保存先・変換設定を選ぶ"] --> Prepare["「変換を確認」で候補と診断を作る"]
    Prepare --> CanSave{"エラーがなく、\nstrict 時は警告もない"}
    CanSave -->|いいえ| Review["診断を確認して設定を直す"]
    Review --> Settings
    CanSave -->|はい| Save["「新規保存」で確認済みの候補を保存する"]
    Save -->|成功| Saved["新しい曲ファイルを保持\n現在曲・未保存編集・履歴は維持"]
    Saved -->|編集対象を切り替えるときだけ「開く」| CanOpen{"現在曲の未保存編集・外部競合がない"}
    CanOpen -->|いいえ| Resolve["保存・再読み込みで解消する\n新規保存済みのファイルは保持"]
    Resolve -->|もう一度「開く」| CanOpen
    CanOpen -->|はい| Open["保存した曲を開く\n開く処理に失敗してもファイルは保持"]
```

MIDI の診断結果で新規保存の可否を判断し、保存後は現在曲の状態を確認して明示的に編集対象を切り替える流れを示す。
