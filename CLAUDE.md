...
- **「何を作るか」も Codex に提案させる**。経路は 2 つ:
  - **実装ランのついで**（日常）: 指示書に `## 提案（任意・このランでは実装しない）` 節を入れ、実装記録の末尾に
    気づきを 3 件まで書かせる。メインが 1 件ずつ採否を判定し、採用は次のランへ回す
  - **独立した提案ラン**（ネタが尽きたとき）: `codex-propose` skill で `docs/proposals/YYYY-MM-DD.md` に 5〜10 案
  - どちらも**提案した Codex にそのまま実装させない**。却下にも理由を書く
