...
{
    /// <summary>Gb 矩形波の周波数・デューティ・音量と加算契約を検証する。</summary>
    public sealed class GbPulseSynthesizerTests
    {
        ...
        {
            var instrument = new GbPulseInstrument { Duty = duty };
            float[] samples = SynthSamples.Render(new GbPulseSynthesizer(), instrument);
        ...
        public void Render_VolumeControlsRootMeanSquare()
        {
            ...
            float[] loud = SynthSamples.Render(new GbPulseSynthesizer(), instrument);
            float[] quiet = SynthSamples.Render(new GbPulseSynthesizer(), instrument, SynthSamples.QuietVolume);
            float[] silent = SynthSamples.Render(new GbPulseSynthesizer(), instrument, volume: 0);
        ...
        public void Render_AddsToBufferAndNoteOffStopsAdding()
        {
            ...
            var instrument = new GbPulseInstrument();
            var reference = new GbPulseSynthesizer();
