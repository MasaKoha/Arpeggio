...
namespace Arpeggio.Core.Session.Sfx
{
    ...
    public static class SfxSessionOutput
    {
        ...
        /// <summary>現在値・保存意図・候補の診断を同じ結果形式へまとめる。</summary>
        public static Dictionary<string, object?> Describe(string operation, Song song,
        ...
        /// <summary>編集後または予行の候補と置換対象・変更一覧を返す。</summary>
        public static object Edit(SfxEditResult result)
        {
        ...
        /// <summary>共通 Catalog の全項目に指定チップでの可否を添える。</summary>
        public static object Schema(ChipKind chip)
        {
        ...
        /// <summary>八用途の説明・対応チップ・全初期値を CLI と MCP 共通で返す。</summary>
        public static object Presets()
        {
            ...
            return new
            {
                ...
                presets = SfxParameterPresetCatalog.GetAll(ChipKind.Nes).Select(preset => new
                {
                    ...
                    {
                        chip, parameters = Parameters(SfxParameterPresetCatalog.Get(preset.Kind, chip).Parameters)
                    }).ToArray()
                }).ToArray()
            };
        }

        /// <summary>保存形式と同じ小文字 enum・非対象チップ省略でパラメータを表す。</summary>
        public static JsonNode? Parameters(SfxParameters? parameters)
        {
            if (parameters == null)
            {
                return null;
