...
namespace Arpeggio.Daw.Presenters
{
    ...
    public sealed class ExportPresenter : IDisposable
    {
        ...
        /// <summary>編集対象・UI 境界・書き出し関数を明示的に結線する。</summary>
        public ExportPresenter(DawDocument document, IMainWindowView view, Action changed,
            Action<Song, string, CancellationToken>? export = null)
        {
        ...
        public async Task RunAsync(string path)
        {
            ...
            try
            {
                ...
                await view.RunOnUiThreadAsync(() => Publish($"書き出し完了: {path}")).ConfigureAwait(false);
            }
        ...
        private void Publish(string text)
        {
            if (isDisposed) { return; }
            StatusText = text;
            view.ShowExportStatus(text, IsRunning);
            changed();
        }
    }
}
